    ...
    for(var i = 0; i < this.sandcastles.length; ++i) {
      ...
      if(!sandcastleData.running && sandcastleData.castle.isInitialized()) {
        ...
        if(nextScript && nextScript.script) {
          this.runOnSandCastle(nextScript.script, nextScript.methodName, nextScript.globals, sandcastleData);
        } else {
          // No scripts on queue.
          break;
        }
...
Pool.prototype.runOnSandCastle = function(nextScript, scriptMethodName, scriptGlobals, sandcastleData) {
  var _this = this;
  ...
  nextScript.super_run(scriptMethodName, scriptGlobals);
}

Pool.prototype.requestRun = function(script, scriptMethodName, scriptGlobals) {
  this.runQueue.push({script: script, methodName: scriptMethodName, globals: scriptGlobals});
...
Pool.prototype.createScript = function(source, opts) {
  ...
  newScript.run = function (methodName, globals) {
    _this.requestRun(newScript, methodName, globals);
  }
  return newScript;
}
